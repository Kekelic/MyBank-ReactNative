...
type BankAccountWithdrawProps = {
  handleBalanceValue: () => void;
  bankAccount: BankAccount;
};

const BankAccountWithdraw = ({
  bankAccount,
  handleBalanceValue,
}: BankAccountWithdrawProps) => {
  ...
  const toggleWithdrawDialog = useCallback(() => {
  ...
  return (
    <View>
      <Button
        ...
        onPress={toggleWithdrawDialog}
      />

      <Dialog
        isVisible={visibleWithdraw}
        onBackdropPress={toggleWithdrawDialog}
        overlayStyle={styles.dialogContainer}
      >
        <Dialog.Title
          title="Isplati s račun"
        ...
        <Controller
          ...
          render={({ field: { value, onChange, onBlur } }) => (
            <Input
              ...
              label="Vrijednost"
              errorMessage={errors.withdrawValue?.message as string}
              value={value}
        ...
        <Dialog.Actions>
          <Dialog.Button
            ...
            onPress={handleSubmit(async ({ withdrawValue }) => {
              try {
                const responseTransfer = await bankApi.patch(
                  `/bank-accounts/${bankAccount.id}/withdraw?amount=${withdrawValue}`,
                  {
                    headers: {
